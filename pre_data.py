...
if __name__ == "__main__":
    import pickle
    trainfile = 'train.txt'
    tokens = findsametok(trainfile)
    dicoftokens = build_dic(tokens)
    dicofids = lookup_dic(tokens)
    f = open(trainfile,'r')
    smi_list1 = f1.readlines()
    f.close()
    maxlength = 0
    minlength = 1000
    tralist = []
    ...
    f.close()
    f = open('tokdict.pickle', 'wb')
    pickle.dump(dicoftokens, f)
    f.close()
    f = open('iddict.pickle','wb')
    pickle.dump(dicofids, f)
    f.close()
